...
void Figure::move(float fElapsedTime) {
	if (!active) return;
	x += vx * speedY(fElapsedTime);
	y += vy * speedY(fElapsedTime);
...
float Figure::speedY(float fElapsedTime) { return (float)(speed*fElapsedTime * (((reduce - 1.) / reduce) * (y - SCREENHEIGHT) / (SCREENHEIGHT - boundary_y) + 1.)); };
