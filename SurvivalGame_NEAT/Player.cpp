...
void Player::draw(sf::RenderWindow& window, bool paused)
{
	...
	int index = (int)std::round((atan2(look_position.y - getY(),
	                                   look_position.x - getX()) + PI) * 8.f / (2.f * PI)) % 8;

	if (!dead_frame_count) {
		if (!paused) shape.sf::RectangleShape::setTexture(shape_default[index].getTexture());
...
std::vector<std::pair<float, float>> Player::closestBots(Figure* figs, unsigned howmany)
{
	Bot* bots = (Bot*)figs;
	...
	for (unsigned i = 0; i < max_bots; ++i) {
		if (!bots[i].getActive()) continue;

		closest[idx].first  = sqrt((bots[i].getX() - getX()) * (bots[i].getX() - getX()) +
		                           (bots[i].getY() - getY()) * (bots[i].getY() - getY()));
		closest[idx].second = atan2((bots[i].getY() - getY()), (bots[i].getX() - getX()));

		if (closest[idx].second < 0) closest[idx].second += 2.*M_PI;

		// std::cout << closest[idx].first << "\t" << closest[idx].second << "\t" << sf::Vector2f(bots[i].getX(),
		// bots[i].getY()) << "\t";
		++idx;
	}
	...
	std::sort(closest.begin(), closest.end(), [](std::pair<float, float> a,
	std::pair<float, float> b) {return a.first < b.first; });
