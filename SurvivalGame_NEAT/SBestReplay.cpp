...
VecD makeAIinput(GameEntities& enti)
{
	...
	for (unsigned i = 0; i < idx_min; ++i) {
		inputs[2 * i] = closest[i].first / distanceMax;
		inputs[2 * i + 1] = closest[i].second / (2. * M_PI);
	}

	for (unsigned i = idx_min; i < howmany; ++i) {
		inputs[2 * i] = distanceMax * 2. / distanceMax;
		inputs[2 * i + 1] = 0.;
	}
        ...
        float fElapsedTime)
{
	...
	float moveY = 0.;
	bool shootPressed = 1;
	// float mouseAngle = 2. * M_PI * inputs[1];
	float mouseAngle = 2. * M_PI * outputs[0];
	float shootX = AI.player.getX() + cos(mouseAngle);
	float shootY = AI.player.getY() + sin(mouseAngle);
	/*
	    float moveX = 0.;
                   ...
                   const GA::Evolver<NEAT::Network>* ga)
{
	...
	while (alive_condition(AI, elapsedTime)) {
		elapsedTime += dt;
		auto out = SBestReplay::stepAI(AI, net, dt);
		fitness += fabs(makeAIinput(AI)[1] * 2. * M_PI - out.first);
	}

	return fitness / elapsedTime;
	// return elapsedTime;
	// return 0;
}
	...
	if (!started) {
		...
		do {
			ga.step();
		         ...
		         (++count_skips < skips || skips == 0) && ga.getBestFitness() < BEST_FITNESS);
	}
...
bool SBestReplay::onCreate()
{
	...
	ga.setEvaluate(fitnessFunc, GA::MINIMIZE);
...
bool SBestReplay::onDraw()
{
	...
	game.player.draw(*renderer, cursor.getPosition(), paused);
	...
	renderer->draw(GF::Text(std::to_string(timer), TOPLEFT_F + sf::Vector2f(10 * SW, 10 * SH),
	                        (int)(SH * 20), GREEN, GF::TOPLEFT));
