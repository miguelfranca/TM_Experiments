...
SBestReplay::SBestReplay(GF::StateMachine* sm, sf::RenderWindow* r) : ga("params.txt"),
	image(nullptr),
	started(false)
{
	renderer = r;
...
VecD makeAIinput(GameEntities& enti)
{
	...
	// if(closest.size()) std::cout << "Closest Distance = " << closest[0].first << std::endl;
	// std::cout << "Player Health = " << enti.player.getHealth() << "\t(" << enti.player.getX() << "," << enti.player.getY() << ")" << std::endl;
...
std::pair<float, bool> SBestReplay::stepAI(GameEntities& AI, const NEAT::Network& net,
        float fElapsedTime)
{
	...
	float moveX = 0.;
	float moveY = 0.;
	bool shotshot = SBestReplay::step(AI, shootPressed, shootX, shootY, moveX, moveY, 0, fElapsedTime);
	// step(AI, ShootPressed, ShootAtX, ShootAtY, HorizontalMovement, VerticlaMovement, ChosenUpgrade, ElapsedTime);
...
double fitnessFunc(const NEAT::Network& net, const GA::Evolver<NEAT::Network>* ga)
{
...
void SBestReplay::newGeneration(bool skip_unchanged, unsigned skips)
{
	...
	if (!started) {
		...
		// do at least one step. Do more steps if skip_unchanged == true ( do 'skips' number of steps)
		do {
			ga.step();

		} while (skip_unchanged && lastFitness == ga.getBestFitness() && ++count_skips < skips
...
bool SBestReplay::onUpdate(const float fElapsedTime, const float fTotalTime)
{
	...
	// std::cout << "(" << game.player.getX() << "," << game.player.getY() << ")" << std::endl;
	// std::cout << game.player.getActive() << std::endl;
	// usleep(100000);
...
bool SBestReplay::onDraw()
{
	...
	renderer->draw(GF::Text(std::to_string(game.player.getScore()),
	                        sf::Vector2f(SW * (110.f + 10.f * digitsS), SH * 20.f),
	                        (int)(SH * 20), BLACK, GF::TOPLEFT));
	renderer->draw(GF::Text("Score:", sf::Vector2f(SW * 50.f, SH * 20.f), (int)(SH * 20), BLACK,
	                        GF::TOPLEFT));
	renderer->draw(GF::Text(std::to_string(game.player.getMoney()),
	                        sf::Vector2f(SW * (35.f + 5.f * digitsM), SH * 54.f),
	                        (int)(SH * 20), YELLOW, GF::TOPLEFT));

	for (int i = 0; i < buttons_number; i++) button[i].draw(*renderer);
