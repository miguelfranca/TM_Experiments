...
VecD Pendulum::invertedPendulumRHS(const VecD &x, const VecD &params)
{
    ...
    return VecD({1, x[3], x[4],
                 ((m * g * sin(x[1]) * cos(x[1]) -
                   m * L * x[3] * x[3] * sin(x[1]) + A * (M + m)) /
                      (M + m * sin(x[1]) * sin(x[1])) * cos(x[1]) +
                  g * sin(x[1])) /
                     L,
                 (m * g * sin(x[1]) * cos(x[1]) -
                  m * L * x[3] * x[3] * sin(x[1]) + A * (M + m)) /
...
Pendulum::Pendulum(sf::RenderTarget *renderer, const std::string indv_net,
                   const sf::Color color)
    : isdead(false), base(sf::Vector2f(300 * SW, 100 * SH),
                          CENTER_WINDOW + sf::Vector2f(0, 200 * SH),
                          GF::CENTERED, color), // setup base car
      line(sf::Vector2f(CENTER_WINDOW + sf::Vector2f(100, 180 * SH)),
           sf::Vector2f(CENTER_WINDOW + sf::Vector2f(100, 180 * SH) -
                        sf::Vector2f(0, toPixels(L))),
           color, color), // setup line
      circle(15,
             sf::Vector2f(CENTER_WINDOW + sf::Vector2f(100, 180 * SH) -
                          sf::Vector2f(0, toPixels(L))),
             color), // setup circle

      eq(Pendulum::invertedPendulumRHS, 4), x0(5),
...
Pendulum::Pendulum(sf::RenderTarget *renderer, const NEAT::Network &indv_net,
                   const sf::Color color)
    : isdead(false), base(sf::Vector2f(300 * SW, 100 * SH),
                          CENTER_WINDOW + sf::Vector2f(0, 200 * SH),
                          GF::CENTERED, color), // setup base car
...
void Pendulum::setAllowUserInput(bool allow) { allowUserInput = allow; }

bool Pendulum::handleEvent(GF::Event &event) { return true; }
...
bool Pendulum::update(const float fElapsedTime, const float fTotalTime)
{
    if (!isdead)
    {
        ...
        bool out = (x0[0] < TMAX_SFML && fabs(x0[1]) <= thetaMAX &&
                    fabs(x0[2]) <= xMAX);
