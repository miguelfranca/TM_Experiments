...
class Pendulum : public GF::Widget
{
  public:
    Pendulum(sf::RenderTarget *renderer = nullptr,
             const std::string indv_net = "",
             const sf::Color = sf::Color::White);
    Pendulum(sf::RenderTarget *renderer, const NEAT::Network &indv_net,
             const sf::Color);

    static VecD invertedPendulumRHS(const VecD &x, const VecD &params);
    static VecD step(VecD x0, double dt, const NEAT::Network &net,
                     ODEsolver &eq, double extra_A = 0.);
    static VecD restart();
