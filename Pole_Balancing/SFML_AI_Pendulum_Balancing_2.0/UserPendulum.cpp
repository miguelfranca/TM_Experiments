...
double UserPendulum::toPixels(double l)
{
    return l / (2. * xMAX) * SCREENWIDTH;
}
double UserPendulum::toMeters(double p)
{
    ...
    : base(sf::Vector2f(300 * SW, 100 * SH),
      ...
      line(sf::Vector2f(CENTER_WINDOW + sf::Vector2f(100, 180 * SH)),
           sf::Vector2f(CENTER_WINDOW + sf::Vector2f(100, 180 * SH) -
                        sf::Vector2f(0, toPixels(L)))), // setup line
      circle(15, sf::Vector2f(CENTER_WINDOW + sf::Vector2f(100, 180 * SH) -
                              sf::Vector2f(0, toPixels(L)))), // setup circle

      eq(UserPendulum::invertedPendulumRHS, 4), x0(5),
...
bool UserPendulum::draw()
{
    ...
    if (base.getPosition().x <= minX || base.getPosition().x >= maxX)
    {
        ...
        base.setPosition((int)SCREENWIDTH * sign + base.getPosition().x,
                         base.getPosition().y);
        line.line[0].position.x =
            (int)SCREENWIDTH * sign + line.line[0].position.x;
        line.line[1].position.x =
            (int)SCREENWIDTH * sign + line.line[1].position.x;
        circle.setPosition((int)SCREENWIDTH * sign + circle.getPosition().x,
