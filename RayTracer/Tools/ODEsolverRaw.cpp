...
bool ODEsolver::checkStop(double t, double T, double dt, const double* lastX) const
{
    bool good;
...
void ODEsolver::setMethod(ODEmethod met)
{
    ...
    case RK45:
        ...
        break;
    default:
        assert(false); // [ODEsolver::setMethod] You shouldn't be here
        break;
    }
}

double* ODEsolver::step(const double* init, double &dt, double derr) const
{
    return (this->*stepFunc)(init, dt, derr);
}

double** ODEsolver::evolve(const double* init, double T, double dt, double derr) const
{
    if (!checkData(init))
    {
        MatrixD temp;
